...
class Program
{
    private static void Main(string[] args)
    {
        if (args.Length == 0)
        {
            ...
            switch (args[0])
            {
                case "--compress" or "-c" when args[1] != null:
                {
                    ...
                    sw.Start();
                    lzw.Compress(args[1],
                        Path.Combine(fileDirectory ?? string.Empty,
                            string.Concat(fileName, "_compressed", fileExtension)));
                    sw.Stop();
                ...
                case "--decompress" or "-d" when args[1] != null:
                {
                    string fileName = Path.GetFileNameWithoutExtension(args[1]);
                    string fileDirectory = Path.GetDirectoryName(args[1]);
                    string fileExtension = Path.GetExtension(args[1]);
                    ...
                    lzw.Decompress(args[1],
                        Path.Combine(fileDirectory ?? string.Empty,
                            string.Concat(fileName, "_decompressed", fileExtension)));
                    sw.Stop();
